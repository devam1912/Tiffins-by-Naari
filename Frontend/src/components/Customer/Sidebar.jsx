...
export default function Sidebar({
  ...
  const { user: reduxUser, token } = useSelector((s) => s.auth);
  ...
  const navItems = [
    { id: "dashboard", icon: <LayoutDashboard size={20} />, label: "Dashboard", path: "/CustomerDashboard", private: true },
    { id: "tiffins", icon: <UtensilsCrossed size={20} />, label: "Browse Tiffins", path: "/tiffins", private: false },
    { id: "subscriptions", icon: <Calendar size={20} />, label: "Subscriptions", path: "/subscriptions", private: true },
    { id: "cart", icon: <ShoppingCart size={20} />, label: "My Cart", path: "/cart", badge: cartItems.length, private: true },
    { id: "order-history", icon: <History size={20} />, label: "Order History", path: "/order-history", private: true },
    { id: "profile", icon: <User size={20} />, label: "My Profile", path: "/CustomerProfile", private: true },
  ];

  const visibleNavItems = navItems.filter(item => !item.private || token);
    ...
    <aside className="sidebar-container" style={{
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .nav-btn span:first-child { display: block !important; margin-bottom: 2px; }
          .nav-btn span:last-child { display: block !important; font-size: 9px !important; text-transform: uppercase; letter-spacing: 0.5px; opacity: 0.8; }
      ...
      <nav className="sidebar-nav" style={{ display: "flex", flexDirection: "column", gap: 4, flex: 1 }}>
        ...
        {!token && (
          <button
            ...
            style={{ justifyContent: collapsed ? "center" : "flex-start", marginTop: "12px", background: "rgba(255,255,255,0.15)", color: "#fff" }}
          >
