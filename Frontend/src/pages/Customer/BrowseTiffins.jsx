...
export default function BrowseTiffins() {
  ...
  // Fetch providers whenever location or radius changes
  useEffect(() => {
  ...
  const StarRating = ({ rating = 0 }) => {
    ...
    return (
      <span style={{ fontSize: 12, letterSpacing: 1 }}>
        {[1, 2, 3, 4, 5].map((s) => (
          <span key={s} style={{ color: s <= stars ? "#f59e0b" : "#d1d5db" }}>★</span>
        ))}
      </span>
    );
  };
      ...
      <style>{`
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .provider-card{
          ...
          border-radius:24px;
          border:1.5px solid rgba(143,174,142,0.2);
        ...
        .provider-card:hover{
          ...
          box-shadow:0 16px 48px rgba(90,120,70,0.18);
          border-color:rgba(143,174,142,0.45);
        }
        .view-menu-btn{
          width:100%;
          padding:13px;
          ...
          box-shadow:0 4px 16px rgba(143,174,142,0.35);
          letter-spacing:0.3px;
        }
        .view-menu-btn:hover{
          opacity:0.9;
          transform:translateY(-1px);
          box-shadow:0 8px 24px rgba(143,174,142,0.45);
        }
        .sort-btn{
          padding:8px 16px;
          border-radius:12px;
        ...
        .skeleton{
          background:linear-gradient(90deg,rgba(143,174,142,0.08) 25%,rgba(143,174,142,0.18) 50%,rgba(143,174,142,0.08) 75%);
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 40px 60px" }}>
        ...
        {loading && (
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 32, ...anim(0) }}>
              ...
              <p style={{ fontFamily: "'Lora',serif", fontSize: 18, color: "#5a7a50", fontWeight: 600 }}>
                Loading nearby tiffin providers...
              </p>
            </div>
            {/* Skeleton cards */}
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "72px 40px", background: "rgba(255,255,255,0.7)", borderRadius: 28, border: "1.5px dashed rgba(143,174,142,0.35)", ...anim(0) }}>
            ...
            <h3 style={{ fontFamily: "'Lora',serif", fontSize: 24, fontWeight: 700, color: "#2d3b2d", marginBottom: 10 }}>
              No tiffin providers found
            </h3>
            <p style={{ color: "#aaa", fontSize: 15, fontWeight: 600, marginBottom: 24 }}>
              {searchQuery
                ? `No results for "${searchQuery}". Try a different search.`
                : `No providers within ${radius} km. Try increasing the radius.`}
            </p>
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
              {searchQuery && (
                <button onClick={() => setSearchQuery("")} style={{ background: "rgba(143,174,142,0.15)", border: "1.5px solid rgba(143,174,142,0.3)", borderRadius: 14, padding: "11px 22px", fontSize: 14, fontWeight: 700, color: "#5a7a50", cursor: "pointer", fontFamily: "'Nunito',sans-serif" }}>
                  Clear search
                </button>
              )}
              {radius < 20 && (
                <button onClick={() => setRadius(r => Math.min(r + 5, 20))} style={{ background: "linear-gradient(135deg,#8FAE8E,#8FA873)", border: "none", borderRadius: 14, padding: "11px 22px", fontSize: 14, fontWeight: 700, color: "#fff", cursor: "pointer", fontFamily: "'Nunito',sans-serif" }}>
            ...
            {filtered.map((p, i) => (
              <div key={p._id} className="provider-card" style={{ animation: `popIn 0.4s cubic-bezier(.22,.68,0,1.2) ${i * 55}ms both` }}>
                ...
                <div style={{ padding: "20px 22px 22px", flex: 1, display: "flex", flexDirection: "column" }}>
                  ...
                  {/* Location */}
                  <p style={{ fontSize: 13, color: "#888", fontWeight: 600, marginBottom: 12, display: "flex", alignItems: "center", gap: 5 }}>
                    <span>📍</span>
                    <span style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {p.area || p.address || "Location not specified"}
                    </span>
                  </p>
                  ...
                  {/* Owner name */}
                  <p style={{ fontSize: 12, color: "#bbb", fontWeight: 600, marginBottom: 16 }}>
                    by {p.ownerName}
                  </p>
